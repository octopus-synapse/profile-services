  ...
  validate(parsedCV: ParsedCV): ValidationResult {
    ...
    if (expIndex !== -1 && eduIndex !== -1 && expIndex > eduIndex) {
      issues.push({
        ...
        message: 'Experience section should typically come before Education for experienced professionals',
        severity: ValidationSeverity.WARNING,
        suggestion: 'Consider placing Experience before Education (unless you are a recent graduate)',
      });
    }
    ...
    return {
      ...
      issues,
      metadata: {
        currentOrder: currentOrder.map(type => CVSectionType[type]),
        recommendedOrder: this.RECOMMENDED_ORDER.map(type => CVSectionType[type]),
      },
    };
  }
