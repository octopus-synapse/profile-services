...
export class PublicationController {
  ...
  @Get()
  @ApiOperation({ summary: 'Get all publications for a resume' })
  ...
  async findAll(
    ...
    return this.publicationService.findAll(resumeId, user.userId, page, limit);
  }
  ...
  async findOne(
    ...
    return this.publicationService.findOne(resumeId, id, user.userId);
  }
  ...
  async create(
    ...
    return this.publicationService.create(resumeId, user.userId, createDto);
  }
  ...
  async update(
    ...
    return this.publicationService.update(resumeId, id, user.userId, updateDto);
  }
  ...
  async remove(
    ...
    return this.publicationService.remove(resumeId, id, user.userId);
  }

  @Post('reorder')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Reorder publications' })
  @ApiParam({ name: 'resumeId', description: 'Resume ID' })
  @ApiResponse({ status: 200, description: 'Publications reordered' })
  async reorder(
    @Param('resumeId', ParseCuidPipe) resumeId: string,
    @CurrentUser() user: UserPayload,
    @Body() reorderDto: ReorderDto,
  ) {
    return this.publicationService.reorder(
      resumeId,
      user.userId,
      reorderDto.ids,
    );
