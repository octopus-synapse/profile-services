...
describe('MecStatsService', () => {
  ...
  describe('getStats', () => {
    ...
    it('should handle null degree gracefully', async () => {
      stubCourseRepo.countCoursesByDegree.mockResolvedValueOnce([
    ...
    it('should fetch all data in parallel', async () => {
      await service.getMecStatistics();

      // All repository methods should be called
      expect(stubInstitutionRepo.countActiveInstitutions).toHaveBeenCalled();
      expect(stubCourseRepo.countActiveCourses).toHaveBeenCalled();
      expect(stubCourseRepo.countCoursesByDegree).toHaveBeenCalled();
      expect(stubInstitutionRepo.countInstitutionsByUf).toHaveBeenCalled();
    });
  });
});
