...
> {
   ...
   * example })` values on the route. That cannot work when the happy path
   * needs a row the probe did not create — a one-shot exchange id, a saved
   * job, a resume owned by the persona. Those routes were red forever, which
   * is worse than being listed: a permanent failure teaches everyone to
   * ignore the suite.
   *
   * Use it ONLY for that. A route failing because its schema is wrong must be
   * fixed, not annotated.
   */
