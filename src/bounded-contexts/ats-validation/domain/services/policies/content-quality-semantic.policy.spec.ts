...
function createCatalogEntry(kind: string, requiredRoles: string[] = []): SectionTypeAtsEntry {
  return {
...
function createItem(kind: string, roles: Array<{ role: string; value: string }>) {
  return {
...
describe('ContentQualitySemanticPolicy', () => {
  ...
  it('warns when required roles from catalog are missing', () => {
    const snapshot: SemanticResumeSnapshot = {
      resumeId: 'resume-1',
      sectionTypeCatalog: [createCatalogEntry('CUSTOM_SECTION', ['TITLE', 'ORGANIZATION'])],
      items: [createItem('CUSTOM_SECTION', [{ role: 'DESCRIPTION', value: 'text' }])],
    };

    const result = policy.validate(snapshot);

    expect(result.issues.some((issue) => issue.code === 'MISSING_REQUIRED_SEMANTIC_ROLES')).toBe(
      true,
    );
    expect(
      result.issues.find((issue) => issue.code === 'MISSING_REQUIRED_SEMANTIC_ROLES')?.message,
    ).toContain('TITLE');
    expect(
      result.issues.find((issue) => issue.code === 'MISSING_REQUIRED_SEMANTIC_ROLES')?.message,
  ...
  it('passes when all required roles from catalog are present', () => {
    const snapshot: SemanticResumeSnapshot = {
      resumeId: 'resume-2',
      sectionTypeCatalog: [createCatalogEntry('MY_SECTION', ['FIELD_A', 'FIELD_B'])],
      items: [
  ...
  it('skips required roles check for kinds not in catalog', () => {
    const snapshot: SemanticResumeSnapshot = {
      ...
      items: [createItem('UNKNOWN_KIND', [{ role: 'DESCRIPTION', value: 'text' }])],
    };
  ...
  it('skips required roles check when catalog entry has no requiredSemanticRoles', () => {
    const snapshot: SemanticResumeSnapshot = {
      ...
      items: [createItem('FLEXIBLE_SECTION', [{ role: 'ANYTHING', value: 'value' }])],
    };
  ...
  it('flags short description regardless of section kind', () => {
    const snapshot: SemanticResumeSnapshot = {
      ...
      items: [createItem('ANY_SECTION', [{ role: 'DESCRIPTION', value: 'Too short' }])],
    };

    const result = policy.validate(snapshot);

    expect(result.issues.some((issue) => issue.code === 'DESCRIPTION_TOO_SHORT')).toBe(true);
  });
  ...
  it('passes description quality when text is long enough', () => {
    ...
    expect(result.issues.some((issue) => issue.code === 'DESCRIPTION_TOO_SHORT')).toBe(false);
  });
});
