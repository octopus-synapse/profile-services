...
import {
  ...
  ResumeIdAndTypeKeyParam,
  ResumeIdParam,
  ResumeSectionItemDataResponseSchema,
  ResumeSectionItemDeletedResponseSchema,
  ResumeSectionsListResponseSchema,
...
export const resumesRoutes: ReadonlyArray<Route<ResumesUseCases>> = [
  {
    ...
    handler: async (ctx, bc) => {
      ...
      return toPaginatedResumesData(result);
    },
  },
  {
    method: 'GET',
  ...
  },
  {
    ...
    openapi: {
      summary: 'Duplicate a resume (snapshot copy that diverges from the source)',
      tags: ['resumes'],
      description: 'Resume CRUD operations',
    },
    sdk: { exported: true },
    handler: async (ctx, bc) => {
      ...
      const body = ctx.body as z.infer<typeof DuplicateResumeBody>;
      const result = await bc.duplicateResumeForUserUseCase.execute(
        ctx.user!.userId,
        resumeId,
        body,
      );
