...
describe('LocaleContent', () => {
  ...
  const ptBrOnlyContent: LocaleContent = {
    defaultLocale: 'pt-BR',
    locales: ['pt-BR'],
    sections: {
      work_experience_v1: {
  ...
  describe('resolveForLocale', () => {
    ...
    it('should fall back to defaultLocale when requested locale is missing from sections', () => {
      const result = resolveForLocale(ptBrOnlyContent, 'en');
      expect(result.locale).toBe('pt-BR');
      expect(result.sections.work_experience_v1.title).toBe('Experiencia Profissional');
    });

    it('should fall back to first available locale when requested locale is absent and default is also absent', () => {
      const content: LocaleContent = {
        defaultLocale: 'en',
        locales: ['en'],
        sections: {
      ...
      expect(result.locale).toBe('en');
      expect(result.sections.summary_v1.title).toBe('Resumo');
    });
  ...
  describe('addLocale', () => {
    it('should add a new locale to existing content', () => {
      ...
      const result = addLocale(ptBrOnlyContent, 'en', newSections);
    ...
    it('should preserve existing locales and sections', () => {
      const newSections: Record<string, LocalizedSection> = {
        work_experience_v1: { title: 'Work Experience' },
      };
    ...
    it('should add new sections that did not exist before', () => {
      ...
      const result = addLocale(ptBrOnlyContent, 'en', newSections);
      expect(result.sections.education_v1?.en?.title).toBe('Education');
      expect(result.sections.work_experience_v1).toBeDefined();
    });

    it('should not mutate the original content', () => {
      const original = structuredClone(ptBrOnlyContent);
      const newSections: Record<string, LocalizedSection> = {
        work_experience_v1: { title: 'Work Experience' },
      };

      addLocale(ptBrOnlyContent, 'en', newSections);
      expect(ptBrOnlyContent).toEqual(original);
    });
  });
