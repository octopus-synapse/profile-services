...
const EnvironmentSchema = z.object({
  ...
  MINIO_PORT: z.coerce.number().int().optional(), // Email (SMTP)
  ...
  EMAIL_FROM_NAME: z.string().default('ProFile'), // Frontend
  FRONTEND_URL: z.string().url().default('http://localhost:3000'),
  UI_BASE_URL: z.string().url().default('http://localhost:3000'), // Logging
  LOG_LEVEL: z.enum(['debug', 'info', 'warn', 'error']).default('info'), // Internal integrations
  INTERNAL_API_TOKEN: z.string().min(1).optional(), // OpenAI (AI features fail at call time if unset)
  OPENAI_API_KEY: z.string().optional(),
  OPENAI_MODEL: z.string().default('gpt-4o-mini'), // SendGrid (fallback for transactional email when SMTP_HOST is not set)
  SENDGRID_API_KEY: z.string().optional(), // LibreTranslate
  LIBRETRANSLATE_URL: z.string().url().default('http://libretranslate:5000'),
  LIBRETRANSLATE_API_KEY: z.string().optional(), // OAuth — GitHub
