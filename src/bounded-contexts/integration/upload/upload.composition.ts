...
export function buildUploadUseCases(s3: S3UploadService, logger: LoggerPort): UploadUseCases {
  ...
  return {
    ...
    deleteUpload: new DeleteUploadUseCase(storage, logger),
  };
}
...
export function buildUploadComposition(
  ...
  return {
    useCases: buildUploadUseCases(s3, logger),
    routes: uploadRoutes,
  };
}
