...
describe('Role Entity', () => {
  describe('create', () => {
    it('should create a role with valid name and displayName', () => {
      const role = Role.create({ name: 'admin', displayName: 'Administrator' });
    ...
    it('should normalize name to lowercase with underscores', () => {
      const role = Role.create({ name: 'Super Admin', displayName: 'Super Administrator' });
    ...
    it('should create a system role when specified', () => {
      const role = Role.create({ name: 'admin', displayName: 'Administrator', isSystem: true });

      expect(role.isSystem).toBe(true);
    });

    it('should set priority when specified', () => {
      const role = Role.create({ name: 'admin', displayName: 'Administrator', priority: 100 });
  ...
  describe('validation', () => {
    it('should reject empty name', () => {
      expect(() => Role.create({ name: '', displayName: 'Administrator' })).toThrow(
        'Role name cannot be empty',
      );
    });

    it('should reject empty displayName', () => {
      expect(() => Role.create({ name: 'admin', displayName: '' })).toThrow(
        'Role displayName cannot be empty',
      );
    });

    it('should reject name starting with number', () => {
      expect(() => Role.create({ name: '1admin', displayName: 'Administrator' })).toThrow(
        'must start with lowercase letter',
      );
    });

    it('should reject name with invalid characters', () => {
      expect(() => Role.create({ name: 'admin-user', displayName: 'Administrator' })).toThrow(
        'must start with lowercase letter',
      );
    });
    ...
    it('should reject name exceeding 50 characters', () => {
      ...
      expect(() => Role.create({ name: longName, displayName: 'Administrator' })).toThrow(
    ...
    it('should reject displayName exceeding 100 characters', () => {
      ...
      expect(() => Role.create({ name: 'admin', displayName: longDisplayName })).toThrow(
        'Role displayName cannot exceed 100 characters',
      );
    });
    ...
    it('should reject description exceeding 500 characters', () => {
      ...
      expect(() =>
        Role.create({ name: 'admin', displayName: 'Administrator', description: longDescription }),
      ).toThrow('Role description cannot exceed 500 characters');
    });

    it('should allow name with underscores', () => {
      const role = Role.create({ name: 'super_admin_user', displayName: 'Super Admin User' });

      expect(role.name).toBe('super_admin_user');
    });

    it('should allow name with numbers after first letter', () => {
      const role = Role.create({ name: 'admin2', displayName: 'Administrator 2' });
  ...
  describe('addPermission', () => {
    it('should add a permission and return new role', () => {
      const original = Role.create({ name: 'admin', displayName: 'Administrator' });
  ...
  describe('removePermission', () => {
    it('should remove a permission and return new role', () => {
      const original = Role.create({ name: 'admin', displayName: 'Administrator' }).addPermission(
        'perm-1',
      );
  ...
  describe('hasPermission', () => {
    it('should return true when role has permission', () => {
      const role = Role.create({ name: 'admin', displayName: 'Administrator' }).addPermission(
        'perm-1',
      );
  ...
  describe('canBeDeleted', () => {
    it('should return true for non-system roles', () => {
      const role = Role.create({ name: 'custom', displayName: 'Custom Role', isSystem: false });
  ...
  describe('update', () => {
    ...
    it('should update priority', () => {
      const original = Role.create({ name: 'admin', displayName: 'Administrator', priority: 0 });
  ...
  describe('setPermissions', () => {
    it('should replace all permissions', () => {
      const original = Role.create({ name: 'admin', displayName: 'Administrator' }).addPermissions([
        'perm-1',
        'perm-2',
      ]);
  ...
  describe('equals', () => {
    it('should return true for roles with same name', () => {
      const role1 = Role.create({ name: 'admin', displayName: 'Administrator 1' });
      const role2 = Role.create({ name: 'admin', displayName: 'Administrator 2' });

      expect(role1.equals(role2)).toBe(true);
    });

    it('should return false for roles with different names', () => {
      const role1 = Role.create({ name: 'admin', displayName: 'Administrator' });
      const role2 = Role.create({ name: 'editor', displayName: 'Editor' });
  ...
  describe('immutability', () => {
    ...
    it('should not modify original role when updating', () => {
      const original = Role.create({ name: 'admin', displayName: 'Original' });
