...
// (legacy 70/20/10 fit-score matcher adapter removed — superseded by the
//  job-match BC's Match Score; external listings are now matchable there.)
...
export function buildJobsUseCases(
  ...
  return {
    ...
    listRecommendedJobs: new ListRecommendedJobsUseCase(
      cache,
    ...
    applyToJob: new ApplyToJobUseCase(jobsRepo, ensureSubmittedEvent, events, logger),
