 ...
 *
 * Keyword analysis and job description matching.
 */
...
export class AnalyzeKeywordsUseCase {
  constructor(private readonly ownership: ResumeOwnershipPort) {}
  ...
  async execute(
    ...
    return this.getKeywordSuggestions(resume, options);
  }
  ...
  async matchJobDescription(
    ...
    await this.ownership.verifyOwnership(resumeId, userId);
    const resume = await this.ownership.getResumeWithDetails(resumeId);
    return this.matchJob(resume, jobDescription);
  }
  ...
  matchJob(resume: ResumeForAnalytics, jobDescription: string): JobMatchResult {
