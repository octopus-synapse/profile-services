 ...
 *
 * Streams real-time notifications to the authenticated user. Mirrors
 * the pattern in `activity-feed-sse.controller.ts`. The
 * create-notification use case (via `EventEmitterNotificationStreamAdapter`)
 * emits to the `notif:user:{ userId }` namespace on the shared
 * EventEmitter bus.
 */
