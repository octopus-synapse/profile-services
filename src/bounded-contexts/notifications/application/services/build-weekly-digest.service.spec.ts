...
import { buildWeeklyDigest } from './build-weekly-digest.service';

const baseInput = {
  userName: 'Enzo',
  stats: { resumeViews: 12, profileViews: 8 },
};
...
describe('buildWeeklyDigest', () => {
  it('returns null when there is nothing to say', () => {
    const result = buildWeeklyDigest({
      userName: 'Enzo',
      stats: { resumeViews: 0, profileViews: 0 },
    });
  ...
  it('produces subject / html / text when at least one stat is non-zero', () => {
    const result = buildWeeklyDigest({
      userName: 'Enzo',
      stats: { resumeViews: 1, profileViews: 0 },
    });
  ...
  it('falls back to a generic greeting when name is missing', () => {
    const result = buildWeeklyDigest({
      userName: null,
      stats: { resumeViews: 5, profileViews: 0 },
    });
