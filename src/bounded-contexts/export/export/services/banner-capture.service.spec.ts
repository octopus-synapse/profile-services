...
describe('BannerCaptureService', () => {
  ...
  describe('error handling', () => {
    it('should handle #banner not found', async () => {
      ...
      await expect(async () => await service.capture()).toThrow('Banner element not found');
      expect(mockPage.close).toHaveBeenCalledTimes(1);
    });
    ...
    it('should close page on navigation error', async () => {
      ...
      await expect(async () => await service.capture()).toThrow('Navigation failed');
      expect(mockPage.close).toHaveBeenCalledTimes(1);
    });
    ...
    it('should close page on selector timeout', async () => {
      ...
      await expect(async () => await service.capture()).toThrow('Timeout waiting for selector');
      expect(mockPage.close).toHaveBeenCalledTimes(1);
    });
    ...
    it('should handle broken logo image (404)', async () => {
      ...
      mockPage.waitForFunction.mockImplementation(
        (_fn: unknown, options?: { timeout?: number }) => {
      ...
      await expect(async () => await service.capture(DEFAULT.PALETTE, logoUrl)).toThrow(
  ...
  describe('URL building', () => {
    ...
    it('should use default host when not configured', async () => {
      ...
      expect(mockPage.goto).toHaveBeenCalledWith(
        expect.stringMatching(new RegExp(`^http://${DEFAULT.HOST}:${DEFAULT.PORT}/`)),
        expect.any(Object),
      );
    });
  ...
  describe('wait for rendering', () => {
    ...
    it('should not wait for logo when no logo URL provided', async () => {
      ...
      const logoWaitCall = waitForSelectorCalls.find((call) => call[0] === '#company-logo');
