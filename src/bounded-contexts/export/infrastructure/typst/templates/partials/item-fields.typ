...
// Mirrors the pattern from resume-latex.service.ts
...
#let subtitle-keys = ("company", "institution", "organization", "issuer")
...
/// Extract URL
#let extract-url(content) = extract-field(content, ("url", "proofUrl", "link", "website", "repositoryUrl"))

/// Format a date string to "Mon Year" format.
/// Input is expected as ISO string or similar parseable format.
#let format-date(date-str) = {
  if date-str == none or date-str == "" { return "" }
  // Typst doesn't have date parsing from arbitrary strings,
  // so we handle common formats manually
  let s = str(date-str)
  // If already in a readable format, return as-is
  if s.len() <= 10 {
    return s
  }
  // Try to extract YYYY-MM from ISO format
...
#let build-date-range(content) = {
  ...
  let end-str = if is-current == true {
    ...
    // If there's a start date but no end date and not marked as current,
    // assume it's a current/ongoing position
    "Presente"
  } else {
    ""
  }
...
/// Extract a single date (for certifications, awards)
#let extract-single-date(content) = {
  let d = extract-field(content, ("issueDate", "date"))
  if d != none { format-date(d) } else { "" }
}
