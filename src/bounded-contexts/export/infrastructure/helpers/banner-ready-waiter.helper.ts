...
export class BannerReadyWaiter {
  ...
  private async debugBannerError(page: Page): Promise<void> {
    ...
    if (bannerMatch) {
      this.logger.error('[BannerCapture] #banner HTML snippet found', { context: 'BannerReadyWaiter' });
    } else {
      const bodyMatch = html.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
      this.logger.error('[BannerCapture] <body> HTML snippet:', { context: 'BannerReadyWaiter', stack: bodyMatch
