...
export class ExportMultiFormatController {
  constructor(private readonly useCases: ExportUseCases) {}

  @RequirePermission(Permission.RESUME_EXPORT)
  @Get(':resumeId/json')
  @ApiOperation({ summary: 'Export resume as JSON' })
  @ApiStreamResponse({ mimeType: 'application/octet-stream', description: 'JSON resume data' })
  ...
  @ApiStreamResponse({ mimeType: 'application/octet-stream', description: 'LaTeX source file' })
  @ApiParam({ name: 'resumeId', description: 'Resume ID' })
  @ApiQuery({
    name: 'template',
    required: false,
