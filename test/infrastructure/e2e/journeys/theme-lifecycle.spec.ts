...
// The legacy `/v1/themes/...` BC was renamed to `resume-styles` with a
// reduced surface (no `/system`, `/popular`, `/apply` endpoints — apply
// lives on `POST /v1/resumes/:resumeId/style`). Until specs are
// rewritten to exercise the new surface, this journey is skipped.
describe.skip('E2E Journey: Theme Lifecycle', () => {
  ...
  describe('Step 2: System themes', () => {
    it('should list seeded system themes', async () => {
      const res = await app.request.get('/api/v1/themes/system').expect(200);
    ...
    it('should get a system theme by ID', async () => {
      const res = await app.request.get(`/api/v1/themes/${systemThemeId}`).expect(200);
  ...
  describe('Step 3: Apply theme to resume', () => {
    ...
    it('should apply a different system theme', async () => {
      const systemRes = await app.request.get('/api/v1/themes/system').expect(200);
  ...
  describe('Step 4: Public theme listing', () => {
    it('should list all themes', async () => {
      const res = await app.request.get('/api/v1/themes?page=1&limit=10').expect(200);
    ...
    it('should return popular themes', async () => {
      const res = await app.request.get('/api/v1/themes/popular').expect(200);
