...
describe('E2E Journey 2: Authentication', () => {
  ...
  describe('Step 3: Token Refresh', () => {
    it('should refresh access token with valid refresh token', async () => {
      ...
      const response = await app.request.post('/api/auth/refresh').send({
        refreshToken,
      });
  ...
  describe('Step 4: Password Validation', () => {
    it('should enforce minimum password length', async () => {
      ...
      const response = await app.request.post('/api/accounts').send(weakPasswordUser);
    ...
    it('should enforce password complexity', async () => {
      ...
      const response = await app.request.post('/api/accounts').send(simplePasswordUser);
  ...
  describe('Step 5: Rate Limiting (if implemented)', () => {
    it('should handle multiple login attempts gracefully', async () => {
      ...
      const attempts = [] as Array<ReturnType<typeof app.request.post>>;
      for (let i = 0; i < 5; i++) {
        attempts.push(
          app.request.post('/api/auth/login').send({
            email: testUser.email,
            password: 'WrongPassword123!',
          }),
        );
      ...
      // RequestBuilder is PromiseLike — wrap with Promise.resolve to await.
      const results = await Promise.all(attempts.map((a) => Promise.resolve(a)));
