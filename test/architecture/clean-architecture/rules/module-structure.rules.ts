...
export function checkControllersDirectory(): RuleResult {
  return runRule(
    'modules MUST have controllers/ directory',
    'Module Structure',
    () => {
...
export function checkServicesDirectory(): RuleResult {
  return runRule(
    'modules MUST have services/ directory',
    'Module Structure',
    () => {
      const violations: string[] = [];
      for (const modulePath of CLEAN_ARCHITECTURE_MODULES) {
...
export function checkModuleFile(): RuleResult {
  return runRule(
    'modules MUST have exactly ONE .module.ts file',
    'Module Structure',
    () => {
      const violations: string[] = [];
      for (const modulePath of CLEAN_ARCHITECTURE_MODULES) {
  ...
  return runRule(
    ...
    () => {
      ...
      for (const modulePath of CLEAN_ARCHITECTURE_MODULES) {
        ...
        const controllerFiles = getFilesInDirectory(
          controllersPath,
          '.controller.ts',
        ).filter((f) => !f.endsWith('.spec.ts'));

        for (const controller of controllerFiles) {
          const specFile = controller.replace(
            '.controller.ts',
            '.controller.spec.ts',
          );
          if (!fileExists(path.join(controllersPath, specFile))) {
            violations.push(
              `${modulePath}/controllers/${controller}: MISSING spec file`,
            );
          }
        }
      }
